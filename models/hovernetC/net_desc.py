...
class HoVerNet(nn.Module):
    ...
    def __init__(self, input_ch=3, nr_types=None, freeze=False, mode='original'):
        ...
        self.fc1=nn.Linear(1024,out_features=1024)
        self.fc2 = nn.Linear(1024,out_features=2)
        # TODO: pytorch still require the channel eventhough its ignored
    ...
    def forward(self, imgs):
        ...
        if self.training:
            ...
            d3 = self.conv_bot(d3)
            d = [d0, d1, d2, d3]
        gap = F.adaptive_avg_pool2d(d3, (1, 1))
        gap=gap.view(-1,1024)
        # d3=d3.view(-1,1024)
        # d3=self.maxpool(d3)
        # d3 = d3.view(-1, 1024 * self.finalwid * self.finalwid)
        fc1=self.fc1(gap)
        fc2 = self.fc2(fc1)
