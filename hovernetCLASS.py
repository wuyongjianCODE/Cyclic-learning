...
from torch.utils.data import DataLoader
from pytorch_grad_cam import GradCAM, HiResCAM, ScoreCAM, GradCAMPlusPlus, AblationCAM, XGradCAM, EigenCAM, FullGrad,EigenGradCAM,LayerCAM,GradCAMElementWise
...
methods=[GradCAM, HiResCAM, GradCAMPlusPlus,  XGradCAM, EigenCAM, FullGrad,EigenGradCAM,LayerCAM,GradCAMElementWise]
...
from models.hovernetC.net_desc import HoVerNet
...
def train(hvnet):
    ...
    # net.load_state_dict(torch.load('logs/ep003.pth', map_location='cpu'))
    print(net)
    if useLRP == True:
        target_layers = [net.d3]
        camslen=len(methods)
        cams=[]
        for i in range(camslen):
            camname=methods[i]
            cam = camname(model=net, target_layers=target_layers, use_cuda=True)
            cams.append(cam)
    ...
    for epoch in range(6,num_epoch):
        ...
        for batch in dataload:
            ...
            if useLRP==True:
                ...
                for i in range(camslen):
                    ...
                    # plt.imshow(oriim)
                    # plt.subplot(2, 2, 4)
                    # plt.imshow(grayscale_cam>0.02)
                    # plt.show()
                    scores=[0.05,0.1,0.15,0.2,0.25,0.3,0.5,0.35,0.35,0.35]
                    ...
                    plt.subplots_adjust(top=1, bottom=0, right=1, left=0, hspace=0, wspace=0)
                    plt.margins(0, 0)
                    plt.imshow(grayscale_cam>scores[i]*np.max(grayscale_cam))
                plt.subplot(5,4,19)
                plt.imshow(imnumpy[0, :, :, :])
                plt.subplot(5,4,20)
                orii=imnumpy[0, :, :, :]
                plt.imshow(orii[:,:,0]<100)
                plt.show()
            # print(f'{pred},,  {label}')
            ...
            correct+=(prediction==torch.argmax(label)).sum().float()
            total+=len(label)
        ...
        print('Acc : {}'.format((correct/total).cpu().detach().data.numpy()))
        torch.save(net.state_dict(), 'logs/ep%03d.pth' % (epochs))
