...
def fix(LLLdir=''):
    ...
    for nam in STAGE_NAMES:
        ...
        import random
        threshold = colours[idd]+(random.random()-0.5)*2*0.01#(np.mean(gray))
        if idd==0:
            threshold = 0.35
        ...
        for i in range(LENGTH+1):
            try:
                IDS.append(con_sqp[rank[-i]].label)
            except:
                break
        ...
        for current_id in IDS:
            ...
            if con_sqp[current_id-1].area<2000 and cid<LENGTH:
                ...
                use[convex == True] = 255
                io.imsave(r'../../../datasets/MoNuSAC/stage1_train/{}'.format(name[:-4])+'/masks/'+'{}.png'.format(name[:-4]+'_'+str(cid)),blank.astype(np.uint8))
                cid = cid + 1
        ...
        for cropid in range(16):
            ...
            io.imsave(SAVPATH+'/{}_crop_{}_all_seeds_step4.png'.format(nam,cropid),mask_crop.astype(np.uint8))
            ...
            # plt.imshow(step3)
            # plt.show()
            io.imsave(SAVPATH + '/{}_crop_{}_all_seeds_step2.png'.format(nam, cropid), step3.astype(np.uint8))
            io.imsave(SAVPATH + '/{}_crop_{}_all_seeds_step3.png'.format(nam, cropid), step2.astype(np.uint8))
            io.imsave(SAVPATH + '/{}_crop_{}_all_seeds_step1.png'.format(nam, cropid), mid_crop.astype(np.uint8))
        idd=idd+1
